...
var __publicField = (obj, key, value) => {
  ...
  const L_AutoGraticule = "";
  const _AutoGraticule = class extends leaflet.LayerGroup {
    constructor(options) {
      super();
    ...
    buildXLine(x) {
      const bottomLL = new leaflet.LatLng(this._bounds.getSouth(), x);
      const topLL = new leaflet.LatLng(this._bounds.getNorth(), x);
      return new leaflet.Polyline([bottomLL, topLL], this.lineStyle);
    }
    buildYLine(y) {
      const leftLL = new leaflet.LatLng(y, this._bounds.getWest());
      const rightLL = new leaflet.LatLng(y, this._bounds.getEast());
    ...
    buildLabel(axis, val) {
      ...
      if (axis == "gridlabel-horiz") {
        latLng = new leaflet.LatLng(bounds.getNorth(), val);
      } else {
        latLng = new leaflet.LatLng(val, bounds.getWest());
      }
    ...
    static bboxIntersect(bbox1, bbox2) {
      const bounds1 = bbox1 instanceof leaflet.LatLngBounds ? bbox1 : leaflet.latLngBounds(bbox1);
      const bounds2 = bbox2 instanceof leaflet.LatLngBounds ? bbox2 : leaflet.latLngBounds(bbox2);
  ...
  __publicField(AutoGraticule, "niceRound", _AutoGraticule.getGridDivisor);
